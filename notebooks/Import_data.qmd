...
```{r setup}
# Load required libraries for ABF file parsing, data manipulation, and visualization
library(readABF)
library(tidyverse)
library(GGally)
library(DT)
...
# Filter protocol-specific records and correctly split file _0010 into control and noradrenalin conditions
All_sweeps <- All_sweeps %>%
    filter(
        file %in% c("_0008", "_0011", "_0013") | file == "_0010"
    ) %>%
    mutate(
        stim = case_when(
            file %in% c("_0008", "_0011", "_0013") ~ if_else(sweep <= 10, "AC", "C"),
            file == "_0010" ~ if_else(sweep <= 16, "AC", "C")
        ),
        cond = case_when(
            file == "_0008" ~ "control",
            file == "_0010" & sweep <= 16 ~ "control",
            file == "_0010" & sweep >= 17 ~ "noradrenalin",
...
```{r passive, fig.width=9, fig.height=6}
...
# Render overview of voltage-step traces
...
```{r qc_filtering, fig.width=9, fig.height=6}
# Define empirical acceptance thresholds for stability diagnostics
baseline_lims <- c(-200, 0)
Rs_lims <- c(0.75 * median(passive_parameters$Rs_MOhm, na.rm = TRUE), 1.25 * median(passive_parameters$Rs_MOhm, na.rm = TRUE))
Rm_lims <- c(0.4 * median(passive_parameters$Rmemb_MOhm, na.rm = TRUE), 2.5 * median(passive_parameters$Rmemb_MOhm, na.rm = TRUE))
Cm_lims <- c(0.4 * median(passive_parameters$Cmemb_pF, na.rm = TRUE), 2.5 * median(passive_parameters$Cmemb_pF, na.rm = TRUE))
...
```{r synaptic_analysis, fig.width=9, fig.height=6}
# Isolate synaptic records within the target time window and filter strictly for C fiber stimulation
...
# Visualize individual synaptic traces overlaid with analysis boundaries
...
print(synaptic_traces_plot)

# Normalize responses and calculate condition-specific averages for comparative visualization
...
print(condition_comparison_plot)
...
```{r correlation_matrix, warning=FALSE, message=FALSE, fig.width=10, fig.height=10}
# Generate pairwise correlation matrix plot using ggpairs with expanded dimensions to ensure visibility
final_dataset |>
...
```{r scatter_trend, fig.width=9, fig.height=6}
